...
<p class="lead">
X-Hub Memory 不是简单的聊天记录、向量检索或长期偏好库，而是一套由 Hub 统一治理的记忆控制面。它把长期真相、上下文装配、权限策略、写入审批、导出门禁和审计证据拆开管理，让 AI 能持续推进长任务，同时不绕开 X 宪章、授权、远端导出门禁和停止权。
</p>
...
这条边界很重要。现实最优路线不是再造一套客户端 memory，而是继续把 memory authority 收进 Rust Hub kernel；Swift 和 XT 保持产品壳、投影、缓存和调用面。
