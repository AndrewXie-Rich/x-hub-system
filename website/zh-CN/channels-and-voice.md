...
<p class="lead">
你想在 Slack 里用 AI。在手机上用。语音用。被 webhook 触发、定时跑、从团队工具发起。但你**也**想要一条审计链——不是七条。这就是 X-Hub 做的、n8n 和 Zapier 没做的事。
</p>

<div class="preview-note">
  <strong>我们不在通道覆盖上赢 n8n。</strong>
...
这也意味着通道和语音不会顺手变成记忆控制中心：它们可以消费受治理的 memory context，但不负责选择 memory executor，也不能直接写 durable memory truth；这条权力仍留在 Hub 控制平面，并通过 `Writer + Gate` 收口。
...
移动确认 latch 是 X-Hub 对 [agent-2fa](https://github.com/AndrewXie-Rich/agent-2fa) 规范的实现——这个规范被抽出来作为独立协议,这样其它 agent runtime 也可以实现同样的配对设备确认 primitive,不必带走 X-Hub 的其余部分。
