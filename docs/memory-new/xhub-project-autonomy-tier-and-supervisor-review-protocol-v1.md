...
- 让 `A4 / OpenClaw` 成为“高自治 + 旁路监督”的模式，而不是“没有 supervisor”。
...
注意：`A0..A4` 是新的 project-level 用户档位，不等于旧的 `AutonomyLevel 1..5`，也不等于 `manual/guided/trusted_openclaw_mode`。
...
| `a4_openclaw` | OpenClaw | `m4_full_scan` | device/browser/connector/extension 受治理执行 | `hybrid` 或 `aggressive` | 战略监督者 / 仲裁者 |
...
冻结规则：
- 如果 `A4` 想用 `device / connector / extension`，必须满足 `trusted_openclaw_mode + trusted automation + device authority + project binding`。
...
- `A4` 若启用 device/connector/extension，必须进入 `trusted_openclaw_mode`
