...
- typed separation between disk model info and loaded instance info, as seen in [ModelInfoBase.ts]($HOME/path/to/workspace/Opensource/lmstudio-js-main/packages/lms-shared-types/src/ModelInfoBase.ts)
- first-class load config, especially `contextLength`, device targeting, progress, and TTL semantics, as seen in [ModelNamespace.ts]($HOME/path/to/workspace/Opensource/lmstudio-js-main/packages/lms-client/src/modelShared/ModelNamespace.ts), [baseModelBackendInterface.ts]($HOME/path/to/workspace/Opensource/lmstudio-js-main/packages/lms-external-backend-interfaces/src/baseModelBackendInterface.ts), [LLMLoadModelConfig.ts]($HOME/path/to/workspace/Opensource/lmstudio-js-main/packages/lms-shared-types/src/llm/LLMLoadModelConfig.ts), and [EmbeddingLoadModelConfig.ts]($HOME/path/to/workspace/Opensource/lmstudio-js-main/packages/lms-shared-types/src/embedding/EmbeddingLoadModelConfig.ts)
- domain-specific model metadata where the model and the loaded instance each expose effective context fields, as seen in [EmbeddingModelInfo.ts]($HOME/path/to/workspace/Opensource/lmstudio-js-main/packages/lms-shared-types/src/embedding/EmbeddingModelInfo.ts)

What to borrow from OpenCode:
- provider-agnostic `providerID + modelID` identity and capability-first model metadata, as seen in [provider.ts]($HOME/path/to/workspace/Opensource/opencode/opencode/packages/opencode/src/provider/provider.ts) and [types.gen.ts]($HOME/path/to/workspace/Opensource/opencode/opencode/packages/sdk/js/src/gen/types.gen.ts)
- explicit `capabilities`, `modalities`, and `limit.context / limit.output` shape for model picker, routing, and UI filtering, as seen in [models.ts]($HOME/path/to/workspace/Opensource/opencode/opencode/packages/opencode/src/provider/models.ts)
- transform-time fail-closed behavior for unsupported modalities instead of silent best effort, as seen in [transform.ts]($HOME/path/to/workspace/Opensource/opencode/opencode/packages/opencode/src/provider/transform.ts)
