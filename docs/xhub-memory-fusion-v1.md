...
## 2) “Markdown 为真相”在 X‑Hub 的落地方式（我们怎么继承 skills ecosystem 的优点）

> 关键点：skills ecosystem 的“Markdown 真相”本质是 *human-readable + diffable + 可修正*。Hub 端可以做到类似，但要考虑加密与权限。
...
路线 B（更像 skills ecosystem）：文件为真相
...
- 这能把 skills ecosystem 的“可控性”引入 Hub 体系

---

## 3) 混合检索（skills ecosystem）如何服务 PD（progressive-disclosure reference architecture）
...
- skills ecosystem 只有“文件检索”，缺少 hooks 自动生成结构化 observation；
- progressive-disclosure reference architecture 有 observation，但 hybrid 检索与本地 sqlite‑vec 这套可嵌入引擎不是其核心；
- X‑Hub 把两者合并：
  - 有证据链（vault）
...
## 4) Pre-compaction Flush：把 skills ecosystem 的“flush”变成 Hub 的后台 job

### 4.1 触发语义在 X‑Hub 的对应关系
skills ecosystem flush 的目标：在 compaction 之前把“会丢失的上下文”写到磁盘。
...
## 5) hooks 自动生成 observations/summaries：我们如何超过 progressive-disclosure reference architecture

progressive-disclosure reference architecture 的优势是“外部观察 + worker 异步压缩”。X‑Hub 可以更强的点：
...
### Phase 1（引入 skills ecosystem Hybrid Index）
- 独立 index DB + 原子重建
