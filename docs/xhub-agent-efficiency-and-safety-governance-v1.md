...
当前冻结的 memory control plane 继续是：用户在 X-Hub 中选择哪个 AI 执行 memory jobs，`Memory-Core` 只作为 governed rule layer，运行时经 `Scheduler -> Worker -> Writer + Gate` 分层执行；本文件里的策略旋钮不重新定义这条权责边界。
...
其中 `memory_write_mode` 只控制 memory promotion / writeback posture，不替代 `memory_model_preferences`，也不决定哪个 AI 执行 memory jobs。
...
边界说明
- 这里的 memory 策略只治理“写入姿态、可见性、跨项目检索、删除权”等策略问题，不重定义 memory executor。
- 真正执行 memory jobs 的 AI 仍由用户在 X-Hub 中选择；`Memory-Core` 仍是规则层；durable writes 仍只经 `Writer + Gate`。
...
效率
- 内置发现（`skills.search`）+ 一键导入（Client Pull + Upload）+ 分层治理（`Memory-Core` 独立规则资产版本链 + `Global/Project` pins）。
