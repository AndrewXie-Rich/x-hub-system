# OpenClaw Skill ABI 兼容契约（`openclaw_skill_abi_compat.v1`）
...
- status: frozen
- machine-readable source: `docs/openclaw_skill_abi_compat.v1.json`
- code anchors:
...
本契约只冻结以下内容：
- OpenClaw manifest -> X-Hub canonical manifest 的字段映射、默认值、拒绝码。
...
导入桥接协议定义见：`docs/openclaw_skill_import_bridge_contract.v1.md`。
...
对应测试：`x-hub/grpc-server/hub_grpc_server/src/skills_store_openclaw_compat.test.js`。
