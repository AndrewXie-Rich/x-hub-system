# X-Hub Memory Hybrid Index（Openclaw Port）v1（可执行规范 / Draft）

- Status: Draft
- Updated: 2026-02-12
- Goal: 把 Openclaw（MIT）里已经验证过的 **“Markdown 为真相 + 混合检索（Vector+FTS）+ 增量索引 + 原子重建 + compaction 前 flush”** 这套能力，以 **可直接落地的工程切片** 方式移植到 X-Hub（Node + SQLite）上，并与我们现有的 5-layer memory / Progressive Disclosure / Memory-Core Policy 对齐。

> 许可提醒：Openclaw 为 MIT，可直接复用代码；必须保留原版权声明与 LICENSE（见“1.2 代码复用清单”）。

---

## 0) 我们到底要从 Openclaw 借什么（结论先行）
...
### 0.2 需要改造的点（Openclaw 原版不适配 X-Hub 的）
- Openclaw 的索引对象是“workspace markdown 文件 + transcript jsonl 文件”；X-Hub 的真实数据源是 **Hub DB（turns/vault/observations/longterm/canonical）**。
- Openclaw 的 FTS query 构建对中文/非 ASCII token 基本失效（`/[A-Za-z0-9_]+/g`）；X-Hub 必须支持中英混搜。
- Openclaw 的嵌入（embeddings）强依赖“外部 provider”；X-Hub 必须在 **secret 不外发** 前提下可用：至少要支持 `local embeddings` 或者对 secret 仅做 FTS。
...
在主仓库新增：
- `third_party/openclaw/`（只放我们实际拷贝/改造的文件子集）
  - `third_party/openclaw/LICENSE`（原文）
  - `third_party/openclaw/NOTICE.md`（写明来源 commit/tag 与改动说明）
...
以下文件在 Openclaw 中已相对独立、可被“抽离成库”：

- Hybrid 检索与得分合并
  - `refer open source/openclaw-cn-2026.1.31/src/memory/hybrid.ts`
  - `refer open source/openclaw-cn-2026.1.31/src/memory/manager-search.ts`

- Chunking/Hash/Embedding parsing
  - `refer open source/openclaw-cn-2026.1.31/src/memory/internal.ts`

- Schema（meta/files/chunks/embedding_cache + fts5）
  - `refer open source/openclaw-cn-2026.1.31/src/memory/memory-schema.ts`

- sqlite-vec 加载
  - `refer open source/openclaw-cn-2026.1.31/src/memory/sqlite-vec.ts`

- Atomic reindex 关键逻辑（可重写也可借思想）
  - `refer open source/openclaw-cn-2026.1.31/src/memory/manager.ts`（`runSafeReindex` 一段）

- Pre-compaction memory flush（触发条件与“只触发一次/周期”）
  - `refer open source/openclaw-cn-2026.1.31/src/auto-reply/reply/memory-flush.ts`
  - `refer open source/openclaw-cn-2026.1.31/src/auto-reply/reply/agent-runner-memory.ts`
  - `refer open source/openclaw-cn-2026.1.31/docs/reference/session-management-compaction.md`

### 1.3 明确不能“照抄”的对象
- Claude-Mem（AGPL）任何实现代码不能拷贝进 X-Hub（MIT 方向）。本 spec 只涉及 Openclaw 代码移植；Claude-Mem 另见 `docs/xhub-memory-progressive-disclosure-hooks-v1.md`。

---

## 2) X-Hub 落地形态：把 Openclaw Index 变成 Hub 的“派生索引层”
...
> 说明：这里“复用 Openclaw schema + 增列”的方式，是为了最大限度复用其成熟实现。
...
写入 meta key（至少）：
- `memory_index_meta_v1`：provider/model/chunking/vector_dims 等（复用 Openclaw）
...
Openclaw 的 `buildFtsQuery()` 只抓英文 token。X-Hub 必须升级：
...
### 4.1 索引输入：Document 抽象（替代 Openclaw 的 FileEntry）
在 X-Hub 侧实现一个抽象（伪结构）：
```ts
type IndexDocument = {
  path: string;
...
说明：
- 可以“直接移植 Openclaw 的 embedding cache”策略：以 `sha256(text)` 作为 cache key。
- `provider_key` 必须只包含 endpoint/model/headers 指纹，不包含 apiKey 明文（Openclaw 已这么做）。

---

## 6) Search：混合检索 API（把 Openclaw 的算法接到 X-Hub）
...
## 8) Pre-compaction Memory Flush：从 Openclaw “借机制”，但换成 X-Hub job

### 8.1 Openclaw 的机制要点（我们要继承的）
