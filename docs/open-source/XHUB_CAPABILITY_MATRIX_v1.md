...
| Route truth | Honest downgrade / fallback / actual-route visibility in X-Terminal | `preview-working` | 运行时真相、fallback、repair log、doctor 读数已在 XT 内形成 working surface，但仍在持续收口。 | `x-terminal/Sources/Hub/`; `x-terminal/Sources/UI/SettingsView.swift`; `x-terminal/Sources/UI/HubSetupWizardView.swift` |
...
| Safe onboarding | Channel onboarding with governed setup / repair path | `preview-working` | onboarding 自动化和 repair path 已形成 working pack，但还未收敛成完全 polished first-run product。 | `x-terminal/work-orders/xt-w3-24-safe-operator-channel-onboarding-automation-implementation-pack-v1.md`; `x-terminal/Sources/UI/HubSetupWizardView.swift` |
...
| Local provider runtime | Transformers / MLX local provider runtime productization | `implementation-in-progress` | 当前是 active theme，文档、Python runtime、Hub app/bridge 都在推进，但尚未进入 validated mainline。 | `docs/xhub-local-provider-runtime-and-transformers-integration-v1.md`; `docs/memory-new/xhub-local-provider-runtime-transformers-work-orders-v1.md`; `x-hub/python-runtime/python_service/` |
...
| Compatibility gates | Proto / skill manifest / package compatibility gates | `implementation-in-progress` | compatibility 方向与部分 doctor/skills compatibility 已有，但还没形成完整的统一 gate discipline。 | `docs/skills_abi_compat.v1.md`; `docs/memory-new/xhub-governed-package-productization-work-orders-v1.md`; `docs/memory-new/xhub-ironclaw-reference-adoption-checklist-v1.md` |
