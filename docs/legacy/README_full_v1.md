...
> Active codebase marker: this repository root is the source of truth for the public X-Hub-System Hub app/control-plane codebase. Build the current Hub UI with `./x-hub/tools/build_hub_app.command`. In this local workspace, active X-Terminal implementation work lives in `/Users/andrew.xie/Documents/AX/rust/rust xt/swift-xterminal` and builds with `/Users/andrew.xie/Documents/AX/rust/rust xt/commands/build_xt.command`; `x-terminal/` is legacy/read-only unless explicitly used for migration or archival comparison. Generated apps and DMGs live under `build/` and are release artifacts, not Git-tracked source.
...
<p>
  <a href="https://xhubsystem.com">Website</a> ·
  <a href="https://xhubsystem.com/get-started">Get started</a> ·
  <a href="https://github.com/AndrewXie-Rich/x-hub-system/releases">GitHub Releases</a>
</p>
...
Under the hood, the Hub-side Swift package still lives in `x-hub/macos/RELFlowHub/` for historical naming reasons. In this local workspace, the active X-Terminal Swift package lives in `/Users/andrew.xie/Documents/AX/rust/rust xt/swift-xterminal`; this repository's `x-terminal/` tree is legacy/read-only unless a task explicitly targets migration or archival comparison. The preferred Hub source-run entrypoint is `x-hub/tools/run_xhub_from_source.command`, and the thin repo-level doctor wrapper is `scripts/run_xhub_doctor_from_source.command`. That wrapper exposes `hub`, `xt`, and `all` modes plus shared `--workspace-root` and `--out-dir` options.
