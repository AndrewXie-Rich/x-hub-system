...
For this local workspace, active implementation roots are split:
...
The in-repo `x-terminal/` tree is legacy/read-only unless a task explicitly asks for migration or archival comparison. `x-hub/macos/RELFlowHub/` is the current Hub Swift shell's historical package name, not proof of a separate old Hub product.
...
If the immediate question is “M0..M4 怎样进入 Rust `/memory/gateway/prepare`”“gateway 现在是不是已经理解 serving_profile_id”“怎么证明 selected/omitted/denied/expanded/fallback-disabled 是机器可读 evidence”，先读 `docs/memory-new/xhub-memory-serving-profile-gateway-alignment-v1.md`，再回到 `docs/memory-new/xhub-memory-serving-profiles-and-adaptive-context-v1.md` 和 `docs/memory-new/xhub-universal-memory-layer-work-orders-v1.md`。
...
For current runtime truth, keep the explainability chain in mind: `ProjectSettingsView` shows `Last Runtime Assembly`, `XTUnifiedDoctor` shows a `Project Context` summary inside `session_runtime_readiness`, `XTUnifiedDoctor` also carries first-class `hubMemoryPromptProjection`, `projectRemoteSnapshotCacheProjection / supervisorRemoteSnapshotCacheProjection`, `heartbeatGovernanceProjection` (including `recoveryDecision`), `durableCandidateMirrorProjection`, `localStoreWriteProjection`, `rustMemoryObjectMutationGateProjection`, `rustMemoryUserRevealGrantProjection`, and `rustMemorySelectionEvidenceProjection` on `session_runtime_readiness`, `skillDoctorTruthProjection` on `skills_compatibility_readiness`, plus `providerKeySelectionProjection`, `providerKeyRouteContextProjection`, and `memoryRouteTruthProjection` on `model_route_readiness`; the generic XT doctor export now mirrors those projections as `project_context_summary`, `hub_memory_prompt_projection`, `project_remote_snapshot_cache_snapshot`, `supervisor_remote_snapshot_cache_snapshot`, `heartbeat_governance_snapshot`, `durable_candidate_mirror_snapshot`, `local_store_write_snapshot`, `rust_memory_object_mutation_gate_snapshot`, `rust_memory_user_reveal_grant_snapshot`, `rust_memory_selection_evidence_snapshot`, `skill_doctor_truth_snapshot`, `provider_key_selection_snapshot`, `provider_key_route_context_snapshot`, and `memory_route_truth_snapshot` instead of leaving them only in raw `detail_lines`. The Rust writeback candidate queue is now also visible in `XTUnifiedDoctor` through `/memory/readiness` as bounded `rust_memory_writeback_candidate_queue_*` detail-lines and typed `rustMemoryWritebackCandidateQueueProjection` JSON for candidate/conflict/stale-review/supersession pressure and noise; `xhubd_daemon.js` ops-report/ops-gate also exports content-free `memory_writeback_candidate_ops_rollup` fields for the same queue diagnostics. Project Settings can trigger Rust-owned candidate maintenance by dry-run preview then apply, and can expand conflict/supersession references by fetching Rust-owned object details for read-only comparison. It also now has a Project Memory Inspector that lists current-project Rust objects through `/memory/objects`, supports basic filters, drops cross-scope returns, hides private/secret previews, expands per-object Rust history through `/memory/objects/{memory_id}/history`, and shows cached Rust model-call-plan selected/omitted/denied/skipped/omitted-reason-count evidence from `memory_gateway_model_call_plan_status/history` without recomputing memory selection. Rust now exposes the object mutation gate (`POST /memory/objects/{memory_id}/archive|delete|pin|unpin`) with archive/delete confirmation, archive-clears-pin behavior, tombstone delete, immutable fail-closed behavior, event history, CLI aliases, and `/memory/readiness.object_store.mutation_gate`; Swift now has caller/evidence wrappers (`HubIPCClient.mutateMemoryObjectViaRust`, `XTMemoryInspectorStore.mutateProjectObject`) plus Project Settings icon controls that call the wrapper, require confirmation for archive/delete, suppress duplicate in-flight actions, refresh from Rust after success, and log bounded mutation evidence only. Rust also exposes the Assistant/User reveal grant gate (`POST /memory/user-reveal-grant/issue|evaluate|revoke`) and `/memory/readiness.object_store.user_reveal_grant`; Swift requests/evaluates/revokes through Rust before listing user-scope shell objects. The Supervisor Personal Memory Center now has a gated Assistant/User Rust object shell that lists only user-scope object metadata after an active Rust grant, fetches per-object detail/history on demand, strips title/text/summary/provenance/policy from Assistant/User shell objects, filters cross-scope returns and history events, and remains absent from Project Settings, Project Coder context, and model-serving paths. Doctor mirrors only content-free grant availability/TTL/authority safety posture. Doctor now projects mutation-gate and user-reveal grant readiness from the cached `/memory/readiness` payload as content-free `rust_memory_object_mutation_gate_*` / `rust_memory_user_reveal_grant_*` lines and typed source/generic JSON without scanning objects or becoming memory authority. Inspector, Doctor, cutover smoke, and daemon ops evidence remain content-free: refresh/history/selection/mutation/grant raw-log entries, Doctor projections, `memory_gateway_cutover_readiness.json`, and `xhubd_daemon.js` rollups carry counts, filters, reason buckets, `omitted_reason_counts`, selected layer/source aggregates, and safety booleans only, while memory IDs, project IDs, prompt/context text, and object content stay out of exported Doctor JSON. These paths keep bounded local evidence only, while Swift remains a display/approval shell rather than memory authority.

For W6 retrieval identity, keep `ref=memory://rust/object/{memory_id}` backward-compatible and treat `chunk_ref`, `chunk_id`, `chunk_identity_schema=xhub.memory.object_chunk_identity.v1`, and line-range fields as additive evidence. The derived `rust_hub_memory_object_index` is now chunk-granular after migration `0009_memory_object_index_chunks.sql`, with `(memory_id, chunk_id)` rows that can be dropped and rebuilt from Rust memory objects. `get_ref` must accept object refs with or without `#chunk_id`; a chunk ref expands through the governed retrieval path to that chunk only. `/memory/gateway/prepare` and `/memory/gateway/model-call-plan` now mirror chunk-level `selected_refs` / `omitted_refs` content-free, prefer the chunk index, and keep model-call `context_text_included=false`. The Rust-refactored XT shell now decodes those cached selected/omitted chunk refs, keeps internal status/history compatible with old cache files, and exposes only content-free chunk counts/schema/line-range shape in Project Memory Inspector and Unified Doctor; UI, raw-log, and Doctor export still avoid full memory IDs, chunk refs, project IDs, prompt/context text, and object content. Daemon ops-report/ops-gate, cutover smoke readiness, and production live stability summaries now also carry content-free chunk rollups (`selected_chunk_count`, `selected_chunk_ref_count`, `omitted_ref_count`, `omitted_chunk_ref_count`, `index_granularity`, `chunk_identity_schema`, `chunk_expand_via_get_ref`) without carrying `selected_refs[]` / `omitted_refs[]` values. Future FTS/vector work should reuse this shape.

Assistant/User memory inspection must remain a separate gated shell, not a widened Project Inspector. `XTMemoryInspectorStore` now has an `XTAssistantUserMemoryInspectorGateSnapshot` / `assistantUserSnapshot` path that defaults fail-closed, labels `scope=user · authority=rust_memory_object_store · Swift shell only`, requires Rust object-store readiness plus an explicit Rust-issued user-scope reveal grant before calling `/memory/objects?scope=user`, and drops any project/cross-scope returns. Rust owns `POST /memory/user-reveal-grant/issue|evaluate|revoke` as a short-TTL, content-free grant envelope with `content_included=false`, `memory_ids_included=false`, `project_coder_allowed=false`, and `production_authority_change=false`; Project Coder / project use-modes are denied. Assistant/User object governance actions (`archive/delete/pin/unpin`) also stay Rust-owned: user-scope mutation requires an active reveal grant id at the Rust mutation gate, uses Supervisor + `assistant_user_memory_inspector`, and returns fail-closed on missing/expired/mismatched grants. `SupervisorPersonalMemoryCenterView` now requests/evaluates/revokes that Rust grant from the Swift shell, shows only readiness/grant badges plus object/drop counts, keeps `content=hidden`, and offers gated mutation controls with archive/delete confirmation; if the user had already opened an object's history, mutation success refreshes bounded Rust history once and otherwise records a content-free `history_not_open_on_demand` status. The action buttons also expose display-only, content-free disabled reason hints derived from Rust readiness/grant/mutation rules and object state; those hints must not include memory IDs, event IDs, object text, or grant policy blobs, and they do not create Swift memory authority. The shell snapshot strips title/text/summary/provenance/policy and does not render user-memory IDs, event IDs, or content. Do not expose personal memory from Project Settings or Project Coder context.
...
The XT export also includes a structured `provider_key_selection_snapshot` on `model_route_readiness` whenever XT has observed a real remote provider-key routing decision for the requested model. Read it as key-routing explainability only: it keeps `requestedProvider / requestedModelId / selectedAccountKey / candidates` machine-readable, but it does not grant XT scheduler authority or direct key-state mutation authority.

The XT export also includes `provider_key_route_context_snapshot` on `model_route_readiness` whenever XT can normalize one stable provider-key route context for the requested model. Read it as route-context explainability only: it keeps `model_id / pool / decision / import_context_lines / import_issues` machine-readable so doctor, export, and troubleshoot surfaces can share the same selected-key plus import-blocker truth without per-surface naming drift or reverse-parsing `detail_lines`. In XT, troubleshoot should treat this route-context snapshot as the primary provider-key contract; older split troubleshoot fields are compatibility-only wrappers. XT should also prefer Hub RPC-backed refresh/cache here, leaving `hub_provider_keys.json` as an explicit compatibility fallback only.
...
This focused smoke now verifies that XT export carries structured `project_context_summary` inside `session_runtime_readiness` plus structured `provider_key_selection_snapshot`, `provider_key_route_context_snapshot`, and `memory_route_truth_snapshot` inside `model_route_readiness`.
...
That gate now emits `build/reports/xhub_doctor_source_gate_summary.v1.json`, `build/reports/xhub_doctor_xt_source_smoke_evidence.v1.json`, and `build/reports/xhub_doctor_all_source_smoke_evidence.v1.json`. The XT smoke evidence now also checks `project_memory_policy`, `project_memory_assembly_resolution`, `hub_memory_prompt_projection`, `project_remote_snapshot_cache_snapshot`, `heartbeat_governance_snapshot`, `provider_key_selection_snapshot`, `provider_key_route_context_snapshot`, `supervisor_memory_policy`, `supervisor_memory_assembly_resolution`, `supervisor_remote_snapshot_cache_snapshot`, and `local_store_write_snapshot`, and the summary keeps compact `project_memory_policy_support`, `project_memory_assembly_resolution_support`, `hub_memory_prompt_projection_support`, `project_remote_snapshot_cache_support`, `heartbeat_governance_support`, `provider_key_selection_support`, `provider_key_route_context_support`, `supervisor_memory_policy_support`, `supervisor_memory_assembly_resolution_support`, and `supervisor_remote_snapshot_cache_support`, so the repo-level doctor chain keeps proving not only project-context, Hub prompt assembly, supervisor review-pressure, provider-key routing truth, provider-key import-blocker truth, supervisor-handoff, and route-truth exports, but also the role-aware memory policy / effective assembly truth behind A-Tier and S-Tier explainability surfaces plus the cache-provenance seam for project/supervisor remote snapshot reuse. `provider_key_selection_support` remains read-only key-routing explainability; it does not upgrade XT into scheduler authority, scope grant authority, or direct key-state mutation authority. `provider_key_route_context_support` remains read-only route-context explainability; it does not upgrade XT into import mutation authority, auth approval authority, or a second scheduler. `hub_memory_prompt_projection_support` remains Hub-first prompt-assembly explainability only; it does not let XT infer prompt contents or weaken constitution / export / policy gates. Those two remote snapshot cache support blocks remain explainability-only cache provenance; they do not upgrade XT cache into durable truth or weaken Hub-first routing.
