...
- purpose: 在保持 `Hub truth-source + XT thin context + fail-closed` 主链不变的前提下，把 Supervisor / X-Terminal 的记忆系统从“固定注入摘要”升级到“固定注入摘要 + 受控按需检索 + 渐进展开 + turn 后生命周期维护”。
- depends_on:
...
- 同步把默认近场窗口升级：
  ...
  - supervisor working set 默认改成 8 user turns
