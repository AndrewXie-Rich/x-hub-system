# X-Terminal OpenClaw Skills 兼容与可靠性提效工单（Spec 严格版）
...
- 本文是 OpenClaw Skills 兼容专项的执行工单，目标是把“可兼容”升级成“可发布、可回滚、可审计”。
...
- 兼容 OpenClaw 常见 skill 包结构、元数据字段、安装心智（本机 add + Hub 纳管）。
...
### Hub-L1：兼容契约与数据模型
- 负责 OpenClaw skill manifest 映射、包规范化、版本兼容矩阵。
...
### 5.1 兼容 KPI
- `openclaw_skill_import_success_rate >= 98%`
- `openclaw_manifest_mapping_coverage = 100%`
...
1. `SKC-W1-01` OpenClaw Skill ABI 兼容契约冻结
...
### SKC-W1-01（P0）OpenClaw Skill ABI 兼容契约冻结

- 目标：冻结 OpenClaw -> X-Hub 的 skill manifest/metadata 映射，避免并行开发语义漂移。
- 泳道：Hub-L1（主）+ XT-L1（协同）。
- 依赖：`SKL-V1-001/002/003/005` 讨论稿。
- 交付物：
  - `openclaw_skill_abi_compat.v1` 契约文档（字段映射、默认值、拒绝码）。
  - 兼容矩阵（supported/partial/blocked）。
  - 审计事件冻结：`skills.package.imported`、`skills.pin.updated`、`skills.revoked`。
- Hub-L1 进展（2026-03-01）：
  - 冻结文档：`docs/openclaw_skill_abi_compat.v1.md`
  - 机读契约：`docs/openclaw_skill_abi_compat.v1.json`
  - 对应实现：`x-hub/grpc-server/hub_grpc_server/src/skills_store.js` + `x-hub/grpc-server/hub_grpc_server/src/services.js`
  - 回归样例：`x-hub/grpc-server/hub_grpc_server/src/skills_store_openclaw_compat.test.js`
- DoD：
  - 映射字段覆盖核心安装/执行路径 100%。
...
- 目标：保持 OpenClaw 本机安装心智，同时提供“一步纳管到 Hub”闭环。
- 泳道：Hub-L1 + XT-L1。
- 依赖：`SKC-W1-01`。
...
- Hub-L1 进展（2026-03-01）：
  - 桥接契约：`docs/openclaw_skill_import_bridge_contract.v1.md`
...
通过标准：以上阻塞项全部转绿前，不得宣称“OpenClaw Skills 兼容 release-ready”。
