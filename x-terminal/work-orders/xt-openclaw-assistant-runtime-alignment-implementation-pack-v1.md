# X-Terminal OpenClaw 上层能力对齐实现包（模型主权继续归 X-Hub）
...
- 本文不是 OpenClaw skills 兼容专项的替代品，而是更上层的“能力吸收实现包”。
- 推进原则固定：
  1. 吸收 `openclaw` 的 assistant runtime / session / onboarding / tool orchestration 思路。
  2. 不吸收 `openclaw` 的模型主权、provider auth、全量 connector 主权。
  3. 所有模型、pairing、grant、device trust、paid model policy 继续只归 `X-Hub`。
- 本文按 `P1 主链 > P2 增强 > No-Go` 排序；P1 未完成前，不允许把“OpenClaw 能力对齐”标记为 release-ready。
...
- 技能可兼容：优先兼容 OpenClaw skills package / manifest / index 形态，而不是整套搬运 runtime。
...
- OpenClaw skill manifest compatibility mapping
...
### `OCA-W1-03` OpenClaw Skills Compatibility Handoff

- priority: `P1`
- owner: `X-Hub` / `X-Terminal`
- gate: `SKC-G1`, `SKC-G2`, `SKC-G4`
- purpose: 与现有 `SKC-*` 系列对齐，先兼容 OpenClaw 风格 skills package / manifest，而不是引 OpenClaw runtime。
...
1. 冻结 OpenClaw manifest -> X-Hub canonical manifest 的字段映射。
...
5. 建立 “OpenClaw compatible skill installed” 的 UI explainability 文案。
6. 补齐 XT 对兼容 package 的 project/global index 展示与冲突提示。

#### 验收

- OpenClaw 风格 package 能被 Hub 规范化并入库。
- XT 能展示并消费对应技能索引。
...
- 在 Hub 内复制一套 OpenClaw 式前台 assistant runtime。
...
3. `OCA-W1-03 OpenClaw Skills Compatibility Handoff`
