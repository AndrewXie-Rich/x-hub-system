...
struct ToolProtocolAssistantSurfaceTests {
    ...
    func assistantSurfaceSpecsExposeSkillAlignedTools() {
        ...
        #expect(ToolPolicy.toolSpec(.supervisorVoicePlayback).contains("status|preview|speak|stop"))
        #expect(ToolPolicy.toolSpec(.supervisorVoicePlayback).contains("text|content|value"))
        ...
        #expect(ToolPolicy.toolSpec(.skills_pin).contains("package_sha256"))
        #expect(ToolPolicy.toolSpec(.skillsExecuteRunner).contains("package_sha256"))
        #expect(ToolPolicy.toolSpec(.summarize).contains("url?|path?|text"))
        #expect(ToolPolicy.toolSpec(.run_local_task).contains("task_kind"))
        #expect(ToolPolicy.toolSpec(.run_local_task).contains("model_id?|model?|preferred_model_id?"))
        #expect(ToolPolicy.toolSpec(.run_local_task).contains("auto-binds a runnable local model"))
    ...
    func toolPolicyGroupsAndRiskIncludeNewSurface() {
        ...
        #expect(allowed.contains(.supervisorVoicePlayback))
        #expect(allowed.contains(.run_local_task))
        #expect(!allowed.contains(.git_push))
        #expect(!allowed.contains(.pr_create))
        #expect(allowed.contains(.skills_search))
        #expect(allowed.contains(.skillsExecuteRunner))
        ...
        #expect(full.contains(.supervisorVoicePlayback))
        #expect(full.contains(.run_local_task))
        #expect(full.contains(.skillsExecuteRunner))
        ...
        #expect(ToolPolicy.risk(for: ToolCall(tool: .skills_pin, args: ["skill_id": .string("find-skills"), "package_sha256": .string("abc")])) == .safe)
        #expect(ToolPolicy.risk(for: ToolCall(tool: .skillsExecuteRunner, args: ["skill_id": .string("echo-skill"), "package_sha256": .string(String(repeating: "a", count: 64))])) == .needsConfirm)
        #expect(ToolPolicy.risk(for: ToolCall(tool: .summarize, args: ["text": .string("hello")])) == .safe)
        #expect(ToolPolicy.risk(for: ToolCall(tool: .run_local_task, args: ["task_kind": .string("embedding"), "model_id": .string("embedder"), "text": .string("hello")])) == .safe)
        ...
        #expect(ToolPolicy.risk(for: ToolCall(tool: .supervisorVoicePlayback, args: [:])) == .safe)
        #expect(ToolPolicy.risk(for: ToolCall(tool: .supervisorVoicePlayback, args: ["action": .string("speak"), "text": .string("test")])) == .safe)
