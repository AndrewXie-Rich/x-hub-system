...
enum UITroubleshootIssue: String, CaseIterable, Codable, Sendable {
    ...
    var summary: String {
        switch self {
        case .grantRequired:
            return "需要付费模型或高风险动作授权时，先确认模型入口，再到 Hub 授权与配额入口修复。"
        case .permissionDenied:
            return "设备能力、系统权限或 Hub policy 拒绝时，不再只显示原始报错，而是直接指到修复面。"
        case .paidModelAccessBlocked:
            return "当设备级 paid model 策略、白名单或预算把请求挡住时，诊断入口必须直接指向 device trust / models / budget，而不是退回无上下文 permission denied。"
        case .hubUnreachable:
        ...
        case .grantRequired:
            return UITroubleshootGuide(
                ...
                steps: [
                    ...
                    UITroubleshootStep(index: 2, instruction: "到 Hub 授权面检查 paid model / web / device capability 与 quota 是否允许该请求。", destination: .hubGrants),
        ...
        case .permissionDenied:
            return UITroubleshootGuide(
                ...
                steps: [
                    ...
                    UITroubleshootStep(index: 2, instruction: "若是系统权限问题，直接打开系统权限；若是能力或 policy 问题，去 Hub grants / security 修复。", destination: .systemPermissions),
        ...
        case .paidModelAccessBlocked:
            return UITroubleshootGuide(
                ...
                steps: [
                    ...
                    UITroubleshootStep(index: 2, instruction: "到 Hub 配对与设备信任页检查该设备是 new_profile 还是 legacy_grant，以及 paid model mode / allowlist 是否允许当前请求。", destination: .hubPairing),
