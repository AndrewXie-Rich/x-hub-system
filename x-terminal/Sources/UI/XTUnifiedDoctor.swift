...
struct XTUnifiedDoctorHubContractProjection: Codable, Equatable, Sendable {
    ...
    init(snapshot: HubContractSnapshot, observedAt: Date = Date()) {
        ...
        self.ok = snapshot.ok
        self.contractReady = false
        self.contractObservedAtMs = snapshot.generatedAtMs ?? checkedAtMs
        self.lastCheckedAtMs = checkedAtMs
        self.lastFetchOK = true
        self.fetchErrorCode = ""
        self.fetchErrorMessage = ""
        self.sourceOfTruth = snapshot.hubProduct.sourceOfTruth ?? ""
        self.kernel = snapshot.hubProduct.kernel ?? ""
        self.shell = snapshot.hubProduct.shell ?? ""
        self.xtRole = snapshot.hubProduct.xtRole ?? ""
        self.mustReadContractFirst = snapshot.xtUpdateRule.mustReadContractFirst ?? false
        self.mustNotRecreateHubAuthorityLocally = snapshot.xtUpdateRule.mustNotRecreateHubAuthorityLocally ?? false
        self.mustFailClosedOnMissingGrantOrStaleContract =
            snapshot.xtUpdateRule.mustFailClosedOnMissingGrantOrStaleContract ?? false
        self.recommendedContractTTLMS = max(0, snapshot.xtUpdateRule.recommendedContractTTLMS ?? 0)
        self.transportSecretFieldsIncluded = snapshot.transportSecurity.secretFieldsIncluded ?? true
        self.remoteXTRequiresPairing = snapshot.transportSecurity.remoteXTRequiresPairing ?? false
        self.remoteXTRequiresMTLSForRuntimeChannels =
            snapshot.transportSecurity.remoteXTRequiresMTLSForRuntimeChannels ?? false
        self.memoryCanonicalWriter = snapshot.capabilities.memory.canonicalWriter ?? ""
        self.memoryDurableTruthInXT = snapshot.capabilities.memory.durableTruthInXT ?? true
        self.skillsAuthority = snapshot.capabilities.skills.authority ?? ""
        self.skillsLeaseRequired = snapshot.capabilities.skills.leaseRequired ?? false
        self.skillsLeaseSourceEndpoint = snapshot.capabilities.skills.leaseSourceEndpoint ?? ""
        self.thirdPartyCodeInHubTrustRoot = snapshot.capabilities.skills.thirdPartyCodeInHubTrustRoot ?? true
        self.skillsPackageHashPinRequired = snapshot.capabilities.skills.packageHashPinRequired ?? false
        self.skillsRevocationEpochRequired = snapshot.capabilities.skills.revocationEpochRequired ?? false
        self.remoteEntryNoDomainSupported = snapshot.capabilities.remoteEntry.supportsNoDomainUsers ?? false
        self.remoteEntryRequiresMTLS = snapshot.capabilities.remoteEntry.requiresMTLS ?? false
        self.modelsXTMustNotSelectPaidProviderDirectly =
            snapshot.capabilities.models.xtMustNotSelectPaidProviderDirectly ?? false
        self.providerRouteSecretFieldsIncluded = snapshot.capabilities.providerRoute.secretFieldsIncluded ?? true
        self.naturalLanguageDirectGrant = snapshot.capabilities.grants.naturalLanguageDirectGrant ?? true
        self.auditFallbackPolicy = snapshot.capabilities.audit.fallbackPolicy ?? ""
        self.contractReady = Self.isReady(
            schemaVersion: self.schemaVersion,
            ok: self.ok,
            sourceOfTruth: self.sourceOfTruth,
            mustReadContractFirst: self.mustReadContractFirst,
            mustNotRecreateHubAuthorityLocally: self.mustNotRecreateHubAuthorityLocally,
            mustFailClosedOnMissingGrantOrStaleContract: self.mustFailClosedOnMissingGrantOrStaleContract,
            transportSecretFieldsIncluded: self.transportSecretFieldsIncluded,
            remoteXTRequiresPairing: self.remoteXTRequiresPairing,
            remoteXTRequiresMTLSForRuntimeChannels: self.remoteXTRequiresMTLSForRuntimeChannels,
            memoryCanonicalWriter: self.memoryCanonicalWriter,
            memoryDurableTruthInXT: self.memoryDurableTruthInXT,
            skillsAuthority: self.skillsAuthority,
            skillsLeaseRequired: self.skillsLeaseRequired,
            skillsLeaseSourceEndpoint: self.skillsLeaseSourceEndpoint,
            thirdPartyCodeInHubTrustRoot: self.thirdPartyCodeInHubTrustRoot,
            skillsPackageHashPinRequired: self.skillsPackageHashPinRequired,
            skillsRevocationEpochRequired: self.skillsRevocationEpochRequired,
            remoteEntryNoDomainSupported: self.remoteEntryNoDomainSupported,
            remoteEntryRequiresMTLS: self.remoteEntryRequiresMTLS,
            modelsXTMustNotSelectPaidProviderDirectly: self.modelsXTMustNotSelectPaidProviderDirectly,
            providerRouteSecretFieldsIncluded: self.providerRouteSecretFieldsIncluded,
            naturalLanguageDirectGrant: self.naturalLanguageDirectGrant,
            auditFallbackPolicy: self.auditFallbackPolicy
        )
    }
...
enum XTUnifiedDoctorBuilder {
    static func build(input: XTUnifiedDoctorInput) -> XTUnifiedDoctorReport {
        ...
        let hubReachability = enrichHubContractSection(
            enrichFreshPairReconnectSmokeSection(
            ...
            projection: input.hubContractProjection,
            hubInteractive: hubInteractive
        )
    ...
    private static func enrichHubContractSection(
        _ section: XTUnifiedDoctorSection,
        projection: XTUnifiedDoctorHubContractProjection?,
        hubInteractive: Bool
    ) -> XTUnifiedDoctorSection {
        guard let projection else { return section }

        section.hubContractProjection = projection
        section.detailLines = orderedUnique(section.detailLines + projection.detailLines())

        let shouldBlockForContract = !projection.contractReady
            && (projection.lastFetchOK || projection.contractObservedAtMs > 0)
        guard hubInteractive, shouldBlockForContract, section.state == .ready else {
            return section
        }

        section.state = .diagnosticRequired
        section.headline = "Hub 可达，但 XT Hub Contract 未就绪"
        section.summary = "XT 已连到 Hub，但还没有拿到可用的 Hub 权威合约。XT 不能自行复制 memory、skills、model route 或 grant 的授权逻辑。"
        section.nextStep = "升级或重启 Hub 后重新运行诊断；确认 Swift 壳的 /xt/hub-contract 能代理到 Rust 内核并返回 xhub.rust_hub.xt_contract.v1。"
