...
enum ToolProfile: String, Codable, CaseIterable {
    case minimal
    case coding
    case full
}
...
struct ToolActionEnvelope: Codable, Equatable {
...
enum ToolPolicy {
    ...
    static func tools(for profile: ToolProfile) -> Set<ToolName> {
        switch profile {
        case .minimal:
            return [.read_file, .list_dir, .search, .git_status, .git_diff, .bridge_status]
        case .coding:
            return [.read_file, .write_file, .list_dir, .search, .run_command, .git_status, .git_diff, .git_apply_check, .git_apply, .bridge_status]
        case .full:
    ...
    static func toolSpec(_ tool: ToolName) -> String {
        ...
        case .search:
            return "- search {pattern, glob?}"
        case .run_command:
            return "- run_command {command, timeout_sec?}"
        case .git_status:
            return "- git_status {}"
        case .git_diff:
        ...
        case .need_network:
            return "- need_network {seconds}"
        case .web_fetch:
            return "- web_fetch {url, timeout_sec?, max_bytes?}"
        }
    }
    ...
    private static func expandPolicyTokens(_ tokens: [String]) -> Set<ToolName> {
        ...
        for token in tokens {
            ...
            case "group:readonly":
                out.formUnion([.read_file, .list_dir, .search, .git_status, .git_diff, .bridge_status])
            case "group:fs":
                out.formUnion([.read_file, .write_file, .list_dir, .search])
            case "group:runtime":
                out.formUnion([.run_command])
            case "group:git":
                out.formUnion([.git_status, .git_diff, .git_apply_check, .git_apply])
            case "group:network":
                out.formUnion([.bridge_status, .need_network, .web_fetch])
            case "group:coding":
