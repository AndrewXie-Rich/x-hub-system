...
enum AXMemoryMarkdown {
    static func render(_ mem: AXMemory) -> String {
        ...
        out.append("- 接手/崩溃恢复：再读最近上下文（`.xterminal/AX_RECENT.md`，建议至少最近 12 轮；来源：`.xterminal/recent_context.json`，兼容旧 `.axcoder/`）。")
        ...
        if let skillsDir = AXSkillsLibrary.ensureSkillsDirectory(),
           let projectDir = AXSkillsLibrary.projectSkillsDir(projectId: projectId, projectName: mem.projectName, skillsDir: skillsDir) {
            ...
            out.append("- skills_dir 不可用：请在 Settings/Import Skills 配置技能库目录，或设置环境变量 `XTERMINAL_SKILLS_DIR`（兼容 `AXCODER_SKILLS_DIR`）。")
        }
    ...
    private static func keyPaths(projectRoot: URL) -> [String] {
        ...
        let modern = projectRoot.appendingPathComponent(".xterminal", isDirectory: true)
        let legacy = projectRoot.appendingPathComponent(".axcoder", isDirectory: true)
        let dataDir: URL = FileManager.default.fileExists(atPath: modern.path) ? modern : legacy
        ...
        let candidates: [String] = [
            ...
            "x-terminal/x-terminal-legacy/AX Coder/tools/build_ax_coder_app.command",
            "x-terminal/x-terminal-legacy/AX Coder/tools/build_x_terminal_app.command",
        ]
