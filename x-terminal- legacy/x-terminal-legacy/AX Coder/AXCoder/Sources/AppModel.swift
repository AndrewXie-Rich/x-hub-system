...
final class AppModel: ObservableObject {
    ...
    private let legacySkillScanLastKey = "axcoder_skill_scan_last_ts"
    private let skillScanHour = 17
    private let skillScanMinute = 30
    private let skillsMigrationKey = "xterminal_skills_migration_v1"
    private let legacySkillsMigrationKey = "axcoder_skills_migration_v1"
    private let hubPairingPortKey = "xterminal_hub_pairing_port"
    private let legacyHubPairingPortKey = "axcoder_hub_pairing_port"
    private let hubGrpcPortKey = "xterminal_hub_grpc_port"
    private let legacyHubGrpcPortKey = "axcoder_hub_grpc_port"
    private let hubInternetHostKey = "xterminal_hub_internet_host"
    private let legacyHubInternetHostKey = "axcoder_hub_internet_host"
    private let hubAxhubctlPathKey = "xterminal_hub_axhubctl_path"
    private let legacyHubAxhubctlPathKey = "axcoder_hub_axhubctl_path"
    private let bridgeAlwaysOnKey = "xterminal_bridge_always_on"
    private let legacyBridgeAlwaysOnKey = "axcoder_bridge_always_on"
    ...
    private let legacySkillsDirDefaultsKey = "axcoder_skills_dir"
    ...
    private func resolveSkillsDirectory() -> URL? {
        let envKeys = ["XTERMINAL_SKILLS_DIR", "AXCODER_SKILLS_DIR"]
        for key in envKeys {
        ...
        for key in [skillsDirDefaultsKey, legacySkillsDirDefaultsKey] {
            if let stored = UserDefaults.standard.string(forKey: key),
               !stored.isEmpty {
                let u = URL(fileURLWithPath: NSString(string: stored).expandingTildeInPath)
                if FileManager.default.fileExists(atPath: u.path) {
                    if key == legacySkillsDirDefaultsKey {
                        UserDefaults.standard.set(u.path, forKey: skillsDirDefaultsKey)
                    }
                    return u
                }
            }
        ...
        // Dev layout: .../x-terminal-legacy/build/X-Terminal.app -> repo root -> AX Coder/skills
        let bundleDir = Bundle.main.bundleURL.deletingLastPathComponent()
        let repoRoot = bundleDir.deletingLastPathComponent()
        let dev = repoRoot.appendingPathComponent("AX Coder/skills", isDirectory: true)
        if FileManager.default.fileExists(atPath: dev.path) {
            return dev
        }

        // Prefer new brand path, but still load legacy data for backward compatibility.
        let supportBase = FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent("Library", isDirectory: true)
            .appendingPathComponent("Application Support", isDirectory: true)
        let candidates = [
            supportBase.appendingPathComponent("X-Terminal", isDirectory: true).appendingPathComponent("skills", isDirectory: true),
            supportBase.appendingPathComponent("AXCoder", isDirectory: true).appendingPathComponent("skills", isDirectory: true),
        ]
        for u in candidates where FileManager.default.fileExists(atPath: u.path) {
            if u.path.contains("/AXCoder/") {
                UserDefaults.standard.set(u.path, forKey: skillsDirDefaultsKey)
            }
            return u
        }
    ...
    private func rememberSkillsDirectory(for url: URL) {
        ...
        UserDefaults.standard.set(dir.path, forKey: legacySkillsDirDefaultsKey)
    }
