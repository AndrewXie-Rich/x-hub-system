...
"""
    ...
    private func slashHubRouteText() -> String {
        ...
        switch mode {
        case .auto:
            behavior = "若已配对 Hub（hub.env 存在）则优先走 gRPC；否则回退本地 file IPC。"
        case .grpc:
            behavior = "仅走 gRPC（要求已配对 Hub）；若未配对会直接报错。"
        case .fileIPC:
            behavior = "仅走本地 file IPC/dropbox。"
        }
        return """
Hub transport:
- mode: \(mode.rawValue)
    ...
    private func performDirectNetworkRequest(
        ...
        Task {
            ...
            if st.enabled {
                ...
                let msg = "已自动批准/已开启网络，剩余约 \(max(1, rem / 60)) 分钟。"
                finalizeTurn(ctx: ctx, userText: userText, assistantText: msg, assistantIndex: assistantIndex)
                return
            }
            ...
            if st2.enabled {
                ...
                let msg = "已自动批准/已开启网络，剩余约 \(mins) 分钟。"
                finalizeTurn(ctx: ctx, userText: userText, assistantText: msg, assistantIndex: assistantIndex)
                return
            }
            ...
            if let out = res?.output.lowercased(), out.contains("auto_approved") {
                let msg = "已自动批准，正在开启网络…"
                finalizeTurn(ctx: ctx, userText: userText, assistantText: msg, assistantIndex: assistantIndex)
                return
            }

            let msg = "已向 Flow Hub 申请联网（\(max(1, reqSeconds / 60)) 分钟）。请在 Hub Inbox 审批。"
            finalizeTurn(ctx: ctx, userText: userText, assistantText: msg, assistantIndex: assistantIndex)
        }
    }
...
"""
    ...
    private func buildToolLoopPrompt(
        ...
        let networkAllowed = allowedTools.contains(.need_network) && allowedTools.contains(.web_fetch)
...
- After network is enabled, use web_fetch to fetch 1-3 authoritative sources, then summarize.
"""
            }
            return """
Networking:
- Network tools are currently disabled by tool policy. Do not emit need_network/web_fetch.
