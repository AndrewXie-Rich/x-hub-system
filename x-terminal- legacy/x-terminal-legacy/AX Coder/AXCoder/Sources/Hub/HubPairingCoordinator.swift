    ...
    func generateRemoteText(
        ...
        if text.isEmpty {
            let reason = nonEmpty(decoded.errorCode)
                ...
                ?? inferFailureCode(from: step.output, fallback: "remote_chat_empty_output")
            return HubRemoteGenerateResult(
                ok: false,
                text: "",
                reasonCode: reason,
        ...
        guard decoded.ok == true else {
            let reason = nonEmpty(decoded.errorCode)
                ?? nonEmpty(decoded.reason)
                ?? nonEmpty(decoded.errorMessage)
                ?? inferFailureCode(from: step.output, fallback: "remote_chat_failed")
            return HubRemoteGenerateResult(
                ok: false,
                text: "",
...
async function main() {
  ...
  const reqId = `grant_web_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
