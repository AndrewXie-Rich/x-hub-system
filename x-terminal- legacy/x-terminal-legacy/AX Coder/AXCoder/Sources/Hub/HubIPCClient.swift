...
enum HubIPCClient {
    ...
    static func requestNetworkAccess(root: URL, seconds: Int, reason: String?) async -> NetworkAccessResult {
        ...
        let routeMode = HubAIClient.transportMode()
        ...
        let preferRemote = hasRemote && (routeMode == .grpc || routeMode == .auto)
    ...
    static func approvePendingGrantRequest(
        ...
        let fallbackReason = routeMode == .grpc ? "hub_env_missing" : "pending_grant_action_not_supported"
        return PendingGrantActionResult(
            ok: false,
            decision: .failed,
            source: "hub_runtime_grpc",
