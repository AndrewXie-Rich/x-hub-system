...
const LABEL = 'com.ax.xhub.route-authority-prep-env';
...
const PREP_COMMAND = path.join(ROOT_DIR, 'tools', 'route_authority_prep_session.command');
...
function usage() {
  return [
    'route_authority_prep_session_launchd.js',
    '',
    'Options:',
    '  --status              Inspect persistent provider/model route prep LaunchAgent',
    '  --install             Install/load LaunchAgent that reapplies prep env at login',
...
function plist(config) {
  const outDir = path.join(ROOT_DIR, 'reports', 'route_authority_prep');
...
<dict>
  ...
  <array>
    ...
    <string>${escapeXml(PREP_COMMAND)}</string>
    <string>--apply</string>
  ...
  <string>${escapeXml(path.join(outDir, 'prep_session_env_launchd.out.log'))}</string>
  <key>StandardErrorPath</key>
  <string>${escapeXml(path.join(outDir, 'prep_session_env_launchd.err.log'))}</string>
</dict>
</plist>
`;
}
...
function runSelfTest() {
  ...
  if (!text.includes('route_authority_prep_session.command')) throw new Error('prep command missing');
...
function kickstart() {
  try {
    ...
    // RunAtLoad is enough; kickstart can fail if launchd already ran it.
  }
}
...
function install(config) {
  if (!fs.existsSync(PREP_COMMAND)) throw new Error(`missing prep command: ${PREP_COMMAND}`);
  fs.mkdirSync(path.dirname(PLIST_PATH), { recursive: true });
  fs.mkdirSync(STATE_DIR, { recursive: true });
  const previous = fs.existsSync(PLIST_PATH) ? fs.readFileSync(PLIST_PATH, 'utf8') : '';
  const backupPath = previous ? `${PLIST_PATH}.rhm078.${Date.now()}.bak` : '';
  ...
  kickstart();
  writeState({
    schema_version: 'xhub.route_authority_prep_session_launchd_state.v1',
...
async function main() {
  ...
  if (config.selfTest) {
    runSelfTest();
    process.stdout.write('route_authority_prep_session_launchd self-test ok\n');
    return;
  }
  ...
  process.stdout.write(`${JSON.stringify({
    ok: true,
    schema_version: 'xhub.route_authority_prep_session_launchd.v1',
    mode: config.mode,
    ...
    prep_env_persistence_installed: status().plist_exists,
...
main().catch((error) => {
  process.stderr.write(`[route_authority_prep_session_launchd] ${error?.stack || error?.message || error}\n`);
  process.exit(1);
});
