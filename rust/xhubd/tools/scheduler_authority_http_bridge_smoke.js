...
async function main() {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'xhub-scheduler-authority-http-bridge-smoke-'));
  ...
  try {
    ...
    const { createSchedulerAuthorityBridge } = await import(pathToFileURL(path.join(srcDir, 'rust_scheduler_authority_bridge.js')).href);
    const warnings = [];
    const logs = [];
    const bridge = createSchedulerAuthorityBridge({
      env: {
        ...
        XHUB_RUST_SCHEDULER_AUTHORITY_HTTP_BASE_URL: baseUrl,
        XHUB_RUST_SCHEDULER_AUTHORITY_HTTP_TIMEOUT_MS: '1000',
        XHUB_RUST_SCHEDULER_AUTHORITY_HTTP_FALLBACK_TO_CLI: '0',
        XHUB_RUST_SCHEDULER_AUTHORITY_OWNER: 'node-authority-http-smoke',
    ...
    const requestId = `scheduler-authority-http-smoke-${process.pid}`;
    const claim = await bridge.claimOnce({
      requestId,
      scopeKey: 'project:scheduler-authority-http-smoke',
      project_id: 'scheduler-authority-http-smoke',
      device_id: 'scheduler-authority-http-smoke',
      payload: { source: 'scheduler_authority_http_bridge_smoke' },
    });
    const released = await bridge.release({ requestId });
    const status = await httpGetJson(`${baseUrl}/scheduler/status?include_queue_items=1&queue_items_limit=5`);
    const ok = claim?.used === true
      && claim?.leased === true
      && released === true
    ...
    fs.writeSync(1, `${JSON.stringify({
      ok,
      schema_version: 'xhub.scheduler_authority_http_bridge_smoke.v1',
      mode: 'node_scheduler_authority_http_first',
      cli_fallback_enabled: false,
      claim: {
        used: claim?.used === true,
        leased: claim?.leased === true,
        run_id: safeString(claim?.runId),
        lease_token_present: Boolean(safeString(claim?.leaseToken)),
        error_code: safeString(claim?.error_code),
      },
      released,
...
try {
  ...
  console.error(`[scheduler_authority_http_bridge_smoke] ${error?.stack || error?.message || error}`);
  process.exit(1);
}
