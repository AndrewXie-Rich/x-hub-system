...
function findNodeProcess() {
  ...
  try {
    ...
    return { pid: 0, root: '', command: '', envCommand: '' };
  }
  const candidates = rows
    ...
    .map(parsePidCommand)
    .filter(Boolean);
  if (candidates.length === 0) return { pid: 0, root: '', command: '', envCommand: '' };
  const parsed = candidates
    .sort((a, b) => b.pid - a.pid)[0];
  const envCommand = readProcessEnvCommand(parsed.pid) || parsed.command;
  return { pid: parsed.pid, root: extractEnvValue(envCommand, ROOT_KEY), command: parsed.command, envCommand };
}
