...
function usage() {
  return [
    'xt_file_ipc_background_watcher_smoke.js',
    '',
    'Options:',
...
function resolveXhubdBinary() {
  ...
  if (built.status !== 0) {
    throw new Error(`cargo build failed before XT file IPC background watcher smoke: ${built.stderr}`);
  }
  return debugBin;
}
...
function writeRequest(baseDir, reqId) {
  ...
  fs.writeFileSync(path.join(baseDir, 'ai_requests', `req_${reqId}.json`), `${JSON.stringify({
    type: 'generate',
    req_id: reqId,
    model_id: 'mlx/background-smoke',
    task_type: 'text_generate',
    prompt: 'hello from xt file ipc background watcher smoke',
    max_tokens: 8,
  })}\n`, 'utf8');
}
...
async function main() {
  ...
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'xhub-xt-file-ipc-background-smoke-'));
  const rootDir = path.join(tempRoot, 'hub-root');
  const ipcDir = path.join(tempRoot, 'xt-ipc');
  const reqId = 'background_smoke';
  let child = null;
  let output = null;
  ...
  try {
    ...
    assertOk(ready.body?.capabilities?.xt_file_ipc_shadow_watcher_background_lifecycle_http === true, 'background watcher capability missing');
    ...
    const start = await httpJson('POST', `${baseUrl}/xt/file-ipc-shadow/watcher-background-start`, {
      base_dir: ipcDir,
      apply: true,
      max_requests: 16,
      max_cycles: 10,
    ...
    assertOk(start.statusCode === 200, 'background start HTTP status failed', start);
    assertOk(start.body?.ok === true, 'background start did not succeed', start.body);
    assertOk(start.body?.ready === false, 'background start marked ready');
    assertOk(start.body?.watcher_background?.background_watcher_started === true, 'background watcher did not start', start.body);
    assertOk(start.body?.watcher_background?.production_file_ipc_ready === false, 'background start marked production ready', start.body);

    const status = await httpJson('POST', `${baseUrl}/xt/file-ipc-shadow/watcher-background-status`, {}, args.timeoutMs);
    assertOk(status.statusCode === 200, 'background status HTTP status failed', status);
    assertOk(status.body?.ok === true, 'background status did not succeed', status.body);
    assertOk(status.body?.watcher_background?.production_authority_change === false, 'background status changed production authority', status.body);

    const stop = await httpJson('POST', `${baseUrl}/xt/file-ipc-shadow/watcher-background-stop`, {}, args.timeoutMs);
    assertOk(stop.statusCode === 200, 'background stop HTTP status failed', stop);
    assertOk(stop.body?.ok === true, 'background stop did not succeed', stop.body);
    assertOk(stop.body?.watcher_background?.active === false, 'background watcher remained active after stop', stop.body);
    ...
    assertOk(!fs.existsSync(lockPath), 'background watcher lock was not released');
    ...
    const report = {
      schema_version: 'xhub.rust_hub.xt_file_ipc_background_watcher_smoke.v1',
      ok: true,
      ...
      checks: {
        ...
        background_start_ok: true,
        background_status_ok: true,
        background_stop_ok: true,
        lock_released: true,
