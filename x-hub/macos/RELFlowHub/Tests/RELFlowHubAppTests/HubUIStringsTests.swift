...
final class HubUIStringsTests: XCTestCase {
    ...
    func testExternalInviteTokenStringsStayCentralized() {
        ...
        XCTAssertEqual(HubUIStrings.Settings.GRPC.copyLocalPairingLink, "复制同 Wi-Fi 配对码")
    ...
    func testSettingsOverviewBadgesAndHighlightsStayLocalized() {
        ...
        XCTAssertEqual(HubUIStrings.Settings.FloatingMode.sectionTitle, "关闭主窗口后显示")
        XCTAssertEqual(HubUIStrings.Settings.FloatingMode.mode, "显示方式")
        XCTAssertEqual(HubUIStrings.Settings.FloatingMode.hidden, "隐藏")
        ...
        XCTAssertEqual(HubUIStrings.Settings.FloatingMode.Size.small, "小")
        XCTAssertEqual(HubUIStrings.Settings.FloatingMode.Size.standard, "标准")
        XCTAssertEqual(HubUIStrings.Settings.FloatingMode.Size.large, "大")
        XCTAssertEqual(FloatingMode.hidden.title, "隐藏")
        XCTAssertEqual(FloatingMode.orb.title, "Orb")
        XCTAssertEqual(FloatingMode.card.title, "卡片")
        XCTAssertEqual(OrbParticleDensity.medium.title, "中")
        XCTAssertEqual(OrbParticleSize.standard.title, "标准")
        ...
        XCTAssertEqual(HubUIStrings.Settings.GRPC.setupHint, "把这些值填到 X-Terminal 的 Hub Setup 页面。外部地址应该是 XT 能通过同网、稳定域名、relay/Spectrum/VPS TCP、公网 IP/DDNS 或 Tailscale 访问到的地址。")
        ...
        XCTAssertEqual(HubUIStrings.Settings.GRPC.inviteLinkNeedsStableHost, "邀请链接需要当前可达的 Hub 地址。可填写同 Wi-Fi / 局域网地址，或稳定域名 / relay / Spectrum / VPS TCP / 公网 IP。")
    ...
    func testModelDrawerAndSheetStringsStayCentralized() {
        ...
        XCTAssertEqual(HubUIStrings.Models.Discover.Lifecycle.pendingVerification, "待验证")
        XCTAssertEqual(HubUIStrings.Models.Discover.Lifecycle.needsReview, "待复检")
        XCTAssertEqual(HubUIStrings.Models.Discover.Lifecycle.verifyAction, "复检")
        XCTAssertEqual(HubUIStrings.Models.Discover.Lifecycle.importedNeedsReview("等待试跑"), "已导入模型库，但当前结果需要复检。等待试跑")
