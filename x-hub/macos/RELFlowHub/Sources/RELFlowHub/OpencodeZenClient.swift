...
enum OpencodeZenClient {
    static let defaultBaseURL = URL(string: "https://opencode.ai/zen/v1")!
    ...
    static func fetchModelIds(apiKey: String, baseURL: URL = defaultBaseURL, timeoutSec: Double = 10.0) async throws -> [String] {
        ...
        guard status >= 200 && status < 300 else {
            ...
            throw NSError(
                domain: "relflowhub",
                code: status,
                userInfo: [NSLocalizedDescriptionKey: "OpenCode Zen /models failed (status=\(status)).\(suffix)"]
            )
        }
