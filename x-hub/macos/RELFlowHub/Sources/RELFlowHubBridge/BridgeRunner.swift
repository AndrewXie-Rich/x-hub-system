...
// Networking-capable helper.
// It is intentionally separated from the core Hub so the core can remain
// system-level "no outgoing connections".
...
final class BridgeRunner {
    ...
    init() {
        ...
        // For ad-hoc/sandbox builds where App Group isn't available, fall back to the
        // public hub directory (/private/tmp/RELFlowHub) so the core Hub + clients can
        // still coordinate with the Bridge.
        baseDir = SharedPaths.appGroupDirectory() ?? SharedPaths.ensurePublicHubDirectory()
