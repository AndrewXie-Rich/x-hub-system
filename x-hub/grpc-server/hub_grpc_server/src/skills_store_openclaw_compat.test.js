...
import {
  loadSkillsIndex,
  normalizeOpenClawSkillManifest,
  setSkillPin,
  uploadSkillPackage,
...
run('missing entrypoint rejects with invalid_manifest', () => {
  assert.throws(
    () => normalizeOpenClawSkillManifest({ skill_id: 'demo.skill', version: '1.0.0' }, { sourceId: 'local:upload' }),
    /invalid_manifest: missing entrypoint\.command/
  );
});

run('legacy aliases map into canonical fields with alias audit markers', () => {
  const mapped = normalizeOpenClawSkillManifest(
    {
