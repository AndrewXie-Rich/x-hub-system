...
function buildFeishuDiscoverySummary(result = {}) {
  ...
  const scopeHint = safeString(ticket.proposed_scope_type) && safeString(ticket.proposed_scope_id)
    ...
    : '';
  return buildFeishuSummaryMessage({
    ...
    lines: [
      'This Feishu conversation is not bound to a governed operator scope yet.',
  ...
  if (Object.keys(xtCommand).length) {
    ...
    return buildFeishuSummaryMessage({
      ...
      fields: [
        ...
        safeString(xtCommand.resolved_device_id || resolvedDeviceId) ? { label: 'Device', value: safeString(xtCommand.resolved_device_id || resolvedDeviceId) } : null,
...
export function buildFeishuResultSummary(result = {}) {
  const discoverySummary = buildFeishuDiscoverySummary(result);
...
export function buildFeishuGrantDecisionSummary({
  event = {},
  binding = {},
} = {}) {
...
export function buildFeishuGrantPendingCard({
  event = {},
  binding = {},
} = {}) {
  const decision = safeString(event.decision).toUpperCase();
...
export async function publishFeishuCommandResult({
  result = {},
  feishu_client = null,
  summary_builder = buildFeishuResultSummary,
} = {}) {
  if (!feishu_client || typeof feishu_client.postMessage !== 'function') {
    return {
      ok: false,
      deny_code: 'feishu_client_invalid',
    };
  }
  const summary = summary_builder(result);
  if (!summary.ok) return summary;
  const delivered = await feishu_client.postMessage(summary.payload);
  return {
    ok: true,
...
export async function publishFeishuGrantDecision({
  event = {},
  binding = {},
  feishu_client = null,
  summary_builder = buildFeishuGrantDecisionSummary,
} = {}) {
  if (!feishu_client || typeof feishu_client.postMessage !== 'function') {
    return {
      ok: false,
...
export async function publishFeishuGrantPending({
  event = {},
  binding = {},
  feishu_client = null,
  summary_builder = buildFeishuGrantPendingCard,
} = {}) {
  if (!feishu_client || typeof feishu_client.postMessage !== 'function') {
    return {
      ok: false,
...
export function createFeishuResultPublisher({
  feishu_client = null,
  summary_builder = buildFeishuResultSummary,
  grant_summary_builder = buildFeishuGrantDecisionSummary,
  grant_pending_builder = buildFeishuGrantPendingCard,
} = {}) {
  return {
    build(result) {
    ...
    async publish(result) {
      return await publishFeishuCommandResult({
        result,
        feishu_client,
        summary_builder,
      });
    ...
    async publishGrantDecision({ event = {}, binding = {} } = {}) {
      return await publishFeishuGrantDecision({
        event,
        binding,
        feishu_client,
    ...
    async publishGrantPending({ event = {}, binding = {} } = {}) {
      return await publishFeishuGrantPending({
        event,
        binding,
        feishu_client,
