...
export function feishuConnectorMetadataFromEnv(env = process.env) {
...
export function feishuConnectorPrincipal({ app_id = 'feishu_operator_adapter' } = {}) {
  return {
    device_id: 'hub_operator_channel_connector',
    user_id: '',
    app_id: safeString(app_id) || 'feishu_operator_adapter',
    project_id: '',
    session_id: '',
  };
}
...
export function createFeishuHubConnectorClient(options = {}) {
  ...
  const metadataFactory = typeof options.metadataFactory === 'function'
    ...
    : (() => feishuConnectorMetadataFromEnv(env));
  const principal = options.principal && typeof options.principal === 'object'
    ? options.principal
    : feishuConnectorPrincipal({ app_id: safeString(options.app_id || 'feishu_operator_adapter') });
