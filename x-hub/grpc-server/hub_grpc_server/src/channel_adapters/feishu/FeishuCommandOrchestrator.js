...
function normalizeFeishuActor(input = {}) {
  const actor = safeObject(input);
  const external_user_id = safeString(actor.external_user_id || actor.open_id || actor.user_id || actor.id);
  const external_tenant_id = safeString(actor.external_tenant_id || actor.tenant_key || actor.account_id);
  return {
    provider: 'feishu',
...
function normalizeFeishuChannel(input = {}) {
  const channel = safeObject(input);
  const provider = normalizeChannelProviderId(channel.provider || 'feishu');
  const scope = safeString(channel.channel_scope || channel.chat_type).toLowerCase();
  return {
    provider: provider || 'feishu',
    account_id: safeString(channel.account_id || channel.tenant_key),
    conversation_id: safeString(channel.conversation_id || channel.chat_id || channel.channel_id),
    thread_key: safeString(channel.thread_key || channel.thread_id || channel.message_id),
    channel_scope: scope === 'p2p' || scope === 'private' || scope === 'dm' ? 'dm' : 'group',
  };
}
...
function normalizeRequestId(source_kind, explicit_request_id, fallback_id, now_ms) {
  ...
  return `feishu:${safeString(source_kind || 'command') || 'command'}:${suffix}`;
}

export function normalizeFeishuCommandInput(input = {}, { now_ms = Date.now() } = {}) {
  const src = safeObject(input);

  if (src.token_valid === false) {
    return {
      ok: false,
  ...
  if (src.action && typeof src.action === 'object') {
    ...
    return {
      ...
      actor: normalizeFeishuActor(src.actor),
      channel: normalizeFeishuChannel(src.channel),
...
export {
  classifyOperatorChannelCommandDispatch as classifyFeishuCommandDispatch,
};

export async function orchestrateFeishuCommand({
  input = {},
  hub_client = null,
  now_fn = Date.now,
} = {}) {
  return await orchestrateOperatorChannelCommand({
    input,
    normalize_input: normalizeFeishuCommandInput,
    hub_client,
    now_fn,
  });
}

export function createFeishuCommandOrchestrator({
  hub_client = null,
  now_fn = Date.now,
} = {}) {
