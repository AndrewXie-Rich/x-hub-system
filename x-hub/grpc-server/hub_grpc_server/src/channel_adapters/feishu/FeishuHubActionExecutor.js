...
export function buildFeishuHubActionExecutionRequest(result = {}) {
  const command = safeObject(result.command);
  return {
...
export function buildFeishuSupervisorBriefProjectionRequest(result = {}) {
  return {
    request_id: safeString(result.request_id),
...
export async function executeFeishuHubAction({
  result = {},
  hub_client = null,
} = {}) {
  ...
  if (shouldUseSupervisorBriefProjection(result)) {
    ...
    const request = buildFeishuSupervisorBriefProjectionRequest(result);
    try {
  ...
  const request = buildFeishuHubActionExecutionRequest(result);
  try {
...
export function createFeishuHubActionExecutor({
  hub_client = null,
} = {}) {
  return {
    buildRequest(result) {
      return buildFeishuHubActionExecutionRequest(result);
    },
    async execute(result) {
      return await executeFeishuHubAction({
        result,
        hub_client,
      });
    },
