...
function statusForDenyCode(deny_code) {
  ...
  if (
    ...
    || code === 'payload_type_unsupported'
    || code === 'event_type_unsupported'
    || code === 'content_type_unsupported'
    || code === 'payload_encrypted_unsupported'
  ) return 400;
  if (code === 'payload_too_large') return 413;
  if (
    code === 'verification_token_invalid'
    || code === 'verification_token_missing'
    || code === 'verification_token_missing_in_payload'
  ) return 401;
  return 403;
}

export function createFeishuIngressHandler(options = {}) {
  const verification_token = safeString(options.verification_token);
  const event_path = safeString(options.event_path || '/feishu/events') || '/feishu/events';
  ...
  return async function handleFeishuIngress(req, res) {
    ...
    const normalized = normalizeFeishuWebhookRequest({
      headers: req?.headers || {},
