...
export function buildWhatsAppCloudHubActionExecutionRequest(result = {}) {
  const command = safeObject(result.command);
  return {
...
export function buildWhatsAppCloudSupervisorBriefProjectionRequest(result = {}) {
  return {
    request_id: safeString(result.request_id),
...
export async function executeWhatsAppCloudHubAction({
  result = {},
  hub_client = null,
} = {}) {
  ...
  if (shouldUseSupervisorBriefProjection(result)) {
    ...
    const request = buildWhatsAppCloudSupervisorBriefProjectionRequest(result);
    try {
  ...
  const request = buildWhatsAppCloudHubActionExecutionRequest(result);
  try {
...
export function createWhatsAppCloudHubActionExecutor({
  hub_client = null,
} = {}) {
  return {
    buildRequest(result) {
      return buildWhatsAppCloudHubActionExecutionRequest(result);
    },
    async execute(result) {
      return await executeWhatsAppCloudHubAction({
        result,
        hub_client,
      });
    },
