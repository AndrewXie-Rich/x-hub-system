...
function withTempReportsDir(fn) {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "xt_w3_33_finalize_test."));
...
run("XT-W3-33 finalize helper closes a scaffolded sample and refreshes the pending report", () => {
  withTempReportsDir((tempDir) => {
    const bundleLib = loadFresh("./xt_w3_33_require_real_bundle_lib.js");
    const prepareHelper = loadFresh("./prepare_xt_w3_33_require_real_sample.js");
    const finalizeHelper = loadFresh("./finalize_xt_w3_33_require_real_sample.js");

    const bundle = bundleLib.readCaptureBundle({
      generatedAt: "2026-03-22T11:00:00Z",
    });
    const sample = bundle.samples[0];
    const prepared = prepareHelper.prepareSampleScaffold(sample, {
      reportsDir: tempDir,
    });
    const scaffoldDir = path.join(tempDir, "xt_w3_33_require_real", sample.sample_id);
    ...
    assert.match(result.report_path, /xt_w3_33_h_require_real_evidence\.v1\.json$/);
    assert.equal(result.qa_gate_verdict, "NO_GO(require_real_samples_pending)");
    assert.equal(result.next_pending_sample_id, "xt_w3_33_rr_02_background_preference_does_not_override_decision_track");
    ...
    assert.equal(updatedSample.operator_notes, "真实运行完成，tech stack 已落盘。");
    assert.equal(updatedSample.decision_audit_ref, "audit_ref_01");
    assert.deepEqual(updatedSample.evidence_refs, [path.join(scaffoldDir, "capture-1.png")]);
    assert.ok(fs.existsSync(path.join(tempDir, "xt_w3_33_h_require_real_evidence.v1.json")));
  });
});
