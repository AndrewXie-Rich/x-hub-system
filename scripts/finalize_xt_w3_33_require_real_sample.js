...
function printUsage(exitCode) {
  const message = [
    ...
    "    --scaffold-dir build/reports/xt_w3_33_require_real/xt_w3_33_rr_01_formal_tech_stack_decision_track_persists",
    "",
    "defaults:",
    "  - status=passed",
    "  - success=true",
...
function finalizeSample(args, nowIso = new Date().toISOString()) {
  ...
  if (!args.skipRegenerateReport) {
    ...
    report = buildRequireRealReport(updatedBundle, {
      shadowStatuses: {
        f: fEvidence?.status,
        g: gEvidence?.status,
      },
...
function main() {
  try {
    ...
    if (output.report_path) {
      ...
      lines.push(`qa_gate_verdict: ${output.qa_gate_verdict}`);
      lines.push(`qa_release_stance: ${output.qa_release_stance}`);
    }
    lines.push(`next_pending_sample_id: ${output.next_pending_sample_id}`);
