...
function spawnOrThrow(command, args, root) {
...
run("oss readiness carries doctor heartbeat governance, durable candidate mirror, and local store write support through release surfaces", () => {
  ...
  try {
    spawnOrThrow(process.execPath, [path.join(__dirname, "generate_hub_r1_release_oss_boundary_report.js")], root);
    spawnOrThrow("python3", [path.join(__dirname, "generate_oss_release_readiness_report.py")], root);

    const readiness = JSON.parse(
      fs.readFileSync(path.join(root, "build/reports/oss_release_readiness_v1.json"), "utf8")
    );
    const scrubReport = JSON.parse(
      fs.readFileSync(path.join(root, "build/reports/oss_secret_scrub_report.v1.json"), "utf8")
    );
    const projectMemoryPolicySupport =
      readiness.checks.reproducibility.doctor_source_gate.project_memory_policy_support;
    const projectMemoryAssemblyResolutionSupport =
      readiness.checks.reproducibility.doctor_source_gate.project_memory_assembly_resolution_support;
    ...
    const supervisorMemoryAssemblyResolutionSupport =
      readiness.checks.reproducibility.doctor_source_gate.supervisor_memory_assembly_resolution_support;
    const supervisorRemoteSnapshotCacheSupport =
      readiness.checks.reproducibility.doctor_source_gate.supervisor_remote_snapshot_cache_support;
    const mirrorSupport =
      readiness.checks.reproducibility.doctor_source_gate.durable_candidate_mirror_support;
    const localStoreWriteSupport =
      readiness.checks.reproducibility.doctor_source_gate.local_store_write_support;
    const xtPairingReadinessSupport =
      readiness.checks.reproducibility.doctor_source_gate.xt_pairing_readiness_support;
    const memoryTruthClosureSupport =
      readiness.checks.reproducibility.doctor_source_gate.xt_memory_truth_closure_support;
    ...
    const scrubMirrorSupport =
      scrubReport.truth_source_boundary.doctor_source_gate.durable_candidate_mirror_support;
    const scrubLocalStoreWriteSupport =
      scrubReport.truth_source_boundary.doctor_source_gate.local_store_write_support;
    const scrubSupervisorRemoteSnapshotCacheSupport =
      scrubReport.truth_source_boundary.doctor_source_gate.supervisor_remote_snapshot_cache_support;
    const scrubXtPairingReadinessSupport =
      scrubReport.truth_source_boundary.doctor_source_gate.xt_pairing_readiness_support;
    const scrubMemoryTruthClosureSupport =
      scrubReport.truth_source_boundary.doctor_source_gate.xt_memory_truth_closure_support;
    const scrubBuildSnapshotInventorySupport =
      scrubReport.truth_source_boundary.doctor_source_gate.build_snapshot_inventory_support;

    assert.equal(readiness.release_stance, "GO");
    assert.equal(
      readiness.checks.security_baseline.xt_ready_evidence_source_ref,
    ...
    assert.deepEqual(scrubProjectRemoteSnapshotCacheSupport.all_source_project_remote_snapshot_cache_snapshot, {
      source: "hub_memory_v1_grpc",
      freshness: "ttl_cache",
      cache_hit: true,
      scope: "mode=project_chat project_id=proj-fixture",
    ...
    assert.deepEqual(scrubHeartbeatSupport.all_source_heartbeat_governance_snapshot, {
    ...
    assert.deepEqual(supervisorRemoteSnapshotCacheSupport.xt_source_supervisor_remote_snapshot_cache_snapshot, {
      source: "hub",
      freshness: "ttl_cache",
      cache_hit: true,
    ...
    assert.deepEqual(
      scrubSupervisorRemoteSnapshotCacheSupport.all_source_supervisor_remote_snapshot_cache_snapshot,
      {
        source: "hub",
        freshness: "ttl_cache",
        cache_hit: true,
    ...
    assert.deepEqual(scrubMirrorSupport.all_source_durable_candidate_mirror_snapshot, {
      status: "local_only",
      target: "hub_candidate_carrier",
      attempted: true,
    ...
    assert.deepEqual(scrubLocalStoreWriteSupport.all_source_local_store_write_snapshot, {
      personal_memory_intent: "manual_edit_buffer_commit",
      cross_link_intent: "after_turn_cache_refresh",
      personal_review_intent: "derived_refresh",
    });
    ...
    assert.equal(
      scrubXtPairingReadinessSupport.all_source_paired_route_set.stable_remote_route.host,
      "hub.tailnet.example"
    );
    assert.equal(
      scrubXtPairingReadinessSupport.xt_source_first_pair_completion_proof.remote_shadow_smoke_status,
      "running"
    );
    assert.equal(memoryTruthClosureSupport.xt_memory_truth_closure_smoke_status, "pass");
    assert.deepEqual(memoryTruthClosureSupport.xt_memory_truth_closure_snapshot.project_context, {
      memory_source: "hub_memory_v1_grpc",
      memory_source_label: "Hub 快照 + 本地 overlay",
      writer_gate_boundary_present: true,
    });
    assert.deepEqual(scrubMemoryTruthClosureSupport.xt_memory_truth_closure_snapshot.canonical_sync_closure, {
    ...
    assert.equal(buildSnapshotInventorySupport.projected_prune_history_count, 0);
    assert.equal(scrubBuildSnapshotInventorySupport.hub.current_snapshot_size_bytes, 209339786);
    assert.equal(scrubBuildSnapshotInventorySupport.xterminal.snapshot_root_exists, true);
    assert.equal(
      readiness.checks.local_service_recovery_support.require_real_focus.blocker_class,
      "current_embedding_dirs_incompatible_with_native_transformers_load"
    );
    assert.equal(
      readiness.checks.local_service_recovery_support.machine_decision.require_real_focus_helper_local_service_recovery_present,
      true
    );
    assert.equal(
      readiness.checks.local_service_recovery_support.require_real_focus.helper_local_service_recovery.top_recommended_action.action_id,
      "enable_lm_studio_local_service"
    );
    assert.deepEqual(
      readiness.checks.local_service_recovery_support.require_real_focus.checked_sources.scan_roots,
      [
    ...
    assert.equal(
      readiness.checks.local_service_recovery_support.require_real_focus.search_recovery.wide_shortlist_search_command.includes("--wide-common-user-roots"),
      true
    );
    assert.equal(
      readiness.checks.local_service_recovery_support.require_real_focus.candidate_acceptance.acceptance_contract.required_gate_verdict,
      "PASS(sample1_candidate_native_loadable_for_real_execution)"
    );
    assert.equal(
      readiness.checks.local_service_recovery_support.require_real_focus.candidate_registration.machine_decision.catalog_write_allowed_now,
      false
    );
    assert.equal(
      readiness.checks.local_service_recovery_support.require_real_focus.candidate_registration.proposed_catalog_entry.id,
      "hf-embed-fixture"
    );
    assert.equal(
      readiness.checks.local_service_recovery_support.require_real_focus.candidate_registration.catalog_patch_plan_summary.blocked_reason,
      "validator_not_pass"
    );
    assert.equal(
      scrubReport.truth_source_boundary.xhub_local_service_operator_recovery.require_real_focus.helper_local_service_recovery.helper_route_contract.helper_route_ready_verdict,
      "NO_GO(helper_bridge_not_ready_for_secondary_reference_route)"
    );
    assert.equal(
      scrubReport.truth_source_boundary.xhub_local_service_operator_recovery.require_real_focus.search_recovery.wide_shortlist_search_command.includes("--wide-common-user-roots"),
      true
    );
    assert.equal(
      readiness.checks.operator_channel_recovery_support.machine_decision.action_category,
      "restore_channel_admin_surface"
    );
    assert.equal(
      readiness.checks.operator_channel_recovery_support.top_recommended_action.action_id,
      "restore_channel_admin_surface"
    );
    assert.match(
      readiness.checks.operator_channel_recovery_support.release_wording.external_status_line,
      /preview-working/
    );
    assert.equal(
      scrubReport.truth_source_boundary.xhub_operator_channel_recovery.onboarding_truth.current_failure_code,
      "channel_runtime_missing"
    );
    assert(
...
run("oss readiness surfaces operator heartbeat governance visibility gaps in compact recovery support", () => {
  const root = createReleaseSurfaceFixture();
  ...
  try {
    ...
    assert.equal(
      readiness.checks.operator_channel_recovery_support.heartbeat_governance_visibility_gap,
      true
    );
    assert.equal(
      readiness.checks.operator_channel_recovery_support.channel_focus_highlight,
      "First smoke proof still lacks heartbeat governance visibility."
    );
    assert.equal(
      scrubReport.truth_source_boundary.xhub_operator_channel_recovery.heartbeat_governance_visibility_gap,
      true
    );
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
...
run("oss readiness prefers strict require-real connector snapshot ref when current snapshot is absent", () => {
  const root = createReleaseSurfaceFixture();
  ...
  try {
    ...
    assert.equal(
      readiness.checks.security_baseline.xt_ready_evidence_mode,
      "require_real_release_chain"
    );
    assert.equal(
      readiness.checks.security_baseline.connector_snapshot_ref,
