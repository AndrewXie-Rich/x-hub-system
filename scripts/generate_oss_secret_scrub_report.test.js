...
run("secret scrub report preserves doctor heartbeat governance, provider key routing, durable candidate mirror, and local store write support", () => {
  ...
  try {
    ...
    spawnOrThrow(process.execPath, [path.join(__dirname, "generate_oss_secret_scrub_report.js")], root);
    ...
    const projectMemoryPolicySupport =
      scrubReport.truth_source_boundary.doctor_source_gate.project_memory_policy_support;
    const projectMemoryAssemblyResolutionSupport =
      scrubReport.truth_source_boundary.doctor_source_gate.project_memory_assembly_resolution_support;
    ...
    const supervisorMemoryPolicySupport =
      scrubReport.truth_source_boundary.doctor_source_gate.supervisor_memory_policy_support;
    const supervisorMemoryAssemblyResolutionSupport =
      scrubReport.truth_source_boundary.doctor_source_gate.supervisor_memory_assembly_resolution_support;
    ...
    assert.equal(scrubReport.verdict, "PASS");
    assert.equal(scrubReport.blockers.length, 0);
    ...
    assert.deepEqual(memoryTruthClosureSupport.xt_memory_truth_closure_snapshot.supervisor_memory, {
      memory_source: "hub",
    ...
    assert.equal(buildSnapshotInventorySupport.largest_reclaim_candidate_surface_id, "hub");
    assert.equal(buildSnapshotInventorySupport.hub.historical_snapshot_count, 2);
    assert.deepEqual(buildSnapshotInventorySupport.xterminal.would_prune_history_refs, []);
  } finally {
