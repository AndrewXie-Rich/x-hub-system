...
function withTempDir(fn) {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "xt_w3_24_n_update_scaffold_test."));
  try {
    fn(tempDir);
  } finally {
...
run("XT-W3-24-N updater collects evidence refs from scaffold dirs while excluding metadata files", () => {
  withTempDir((tempDir) => {
...
run("XT-W3-24-N updater can derive sample id, template and evidence refs from scaffold dir", () => {
  withTempDir((tempDir) => {
    const updater = loadFresh("./update_xt_w3_24_n_whatsapp_cloud_require_real_capture_bundle.js");
    const scaffoldDir = path.join(tempDir, "sample");
    ...
    assert.equal(applied.sampleId, "xt_w3_24_n_rr_03_deploy_plan_routes_project_first_to_preferred_xt");
...
run("XT-W3-24-N updater keeps native JSON field types and rejects placeholder scaffold values", () => {
  withTempDir(() => {
    const updater = loadFresh("./update_xt_w3_24_n_whatsapp_cloud_require_real_capture_bundle.js");
    const bundle = {
      ...
      execution_order: ["xt_w3_24_n_rr_04_grant_approve_requires_pending_scope_match"],
      samples: [
        {
    ...
    assert.throws(() => updater.updateBundle(bundle, {
      sampleId: "xt_w3_24_n_rr_04_grant_approve_requires_pending_scope_match",
      status: "passed",
      success: true,
      ...
      setFields: {
        ...
        structured_action_name: "grant.approve",
        pending_grant_owned_by_scope: true,
        grant_scope_matched: true,
        grant_decision: "approved",
        action_audit_ref: "<action_audit_ref>",
        audit_chain_complete: true,
        evidence_origin: "real_runtime",
        synthetic_runtime_evidence: false,
        synthetic_markers: [],
      },
    }, "2026-03-22T10:05:00Z"), /machine_readable_field_placeholder:action_audit_ref/);
    ...
    const result = updater.updateBundle(bundle, {
      ...
      setFields: {
        ...
        grant_scope_matched: true,
        grant_decision: "approved",
        action_audit_ref: "audit-action-001",
        audit_chain_complete: true,
        evidence_origin: "real_runtime",
        synthetic_runtime_evidence: false,
        synthetic_markers: [],
    ...
    assert.equal(result.sample.action_audit_ref, "audit-action-001");
    assert.equal(result.sample.pending_grant_owned_by_scope, true);
