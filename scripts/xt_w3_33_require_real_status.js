...
function printUsage(exitCode) {
  const message = [
    "usage:",
    "  node scripts/xt_w3_33_require_real_status.js",
    "  node scripts/xt_w3_33_require_real_status.js --all",
    "  node scripts/xt_w3_33_require_real_status.js --sample-id xt_w3_33_rr_03_low_risk_default_proposal_stays_pending",
    "  node scripts/xt_w3_33_require_real_status.js --json",
    "",
  ].join("\n");
...
function printHuman(summary) {
  const lines = [];
  lines.push("XT-W3-33 require-real status");
  ...
  for (const row of summary.fg_shadow_statuses) {
    lines.push(`${row.item}: ${row.current_status}`);
  }
  lines.push(
